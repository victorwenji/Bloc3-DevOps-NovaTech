...
5. `deploy-staging` (`.github/workflows/deploy.yml`, `environment: staging`) :
   ...
   - Déploiement du gateway avec **vérification automatisée du zero-downtime** (sauf premier
     déploiement sur un environnement neuf) : un pod dédié tape `/health` en continu pendant tout
     le rolling update ; la moindre requête en échec fait échouer le pipeline. Voir
     `docs/BLUE-GREEN-STRATEGIE.md` pour le détail de cette stratégie (Rolling Update explicite,
     retenue comme alternative au Blue/Green).
